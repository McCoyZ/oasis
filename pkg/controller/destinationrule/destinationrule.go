...
// DestinationRuleController Contact ServicePolicy
type DestinationRuleController struct {
	// 客户端
	client                clientset.Interface
	destinationRuleClient istioclient.Interface
...
func (v *DestinationRuleController) syncService(key string) error {
	...
	subsets := make([]*networkingv1beta1api.Subset, 0)
	for _, deployment := range deployments {
		...
		subset := &networkingv1beta1api.Subset{
	...
	if err != nil {
		if errors.IsNotFound(err) {
			currentDestinationRule = &networkingv1beta1.DestinationRule{
...
func (v *DestinationRuleController) handleErr(err error, key interface{}) {
	...
	if v.queue.NumRequeues(key) < maxRetries {
		log.V(2).Info("Error syncing destinationrule for service, retrying.", "key", key, "error", err)
